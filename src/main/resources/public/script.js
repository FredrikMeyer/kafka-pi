...
const opts = {
    title: "Pi estimation", width: 500, height: 600, pxAlign: false, scales: {
        y: {
            //	auto: false,
            range: [0, 4],
        }, x: {
        ...
        scale: "%",
        side: 1,
        values: (u, vals, space) => vals.map(v => new Intl.NumberFormat('no-NO', {notation: "engineering"}).format(v)),
        size: 100
    }], series: [{}, {
        label: "Sine", stroke: "red", scale: "Abs", spanGaps: true,
    }, {
        label: "Error", stroke: "blue", scale: "%", spanGaps: true,
    },],
};
...
socket.addEventListener("message", (event) => {
    ...
    document
        .getElementById("current-val").innerText = `Current estimation: ${lastData.estimation}. Error: ${lastData.error}`
});
